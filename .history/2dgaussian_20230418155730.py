...
@torch.no_grad()
def getGaussianKernel(ksizes, sigmas):
    # if sigma <= 0:# 根据 kernelsize 计算默认的 sigma，和 opencv 保持一致        
    kernels = []
    for ksize, sigma in enumerate(zip(ksizes, sigmas)):
        # sigma = 0.3 * ((ksize - 1) * 0.5 - 1) + 0.8     
        center = ksize // 2    
        xs = (np.arange(ksize, dtype=np.float32) - center) # 元素与矩阵中心的横向距离 

        Dckernel1d = np.exp(-(xs ** 2) / (2 * sigma ** 2)) # 计算一维卷积核# 根据指数函数性质，利用矩阵乘法快速计算二维卷积核    
        Dskernel1d = np.exp()

        kernel = kernel1d[..., None] @ kernel1d[None, ...]     
        kernel = torch.from_numpy(kernel)    
        kernels.append(kernel / kernel.sum()) # 归一化return kernel
    return kernels


def GaussianBlur(batch_img, ksize, sigmac, sigmas):    
    kernels = getGaussianKernel(ksize, sigma) # 生成权重积核    
