...
def CVGaussianBlur(batch_img, ksize):
    sigma = 0.3 * ((ksize - 1) * 0.5 - 1) + 0.8
    kernel_1d = cv2.getGaussianKernel(ksize=5, sigma=sigma, ktype=cv2.CV_32F)
    kernel_2d = kernel_1d * kernel_1d.T
    B, C, H, W = batch_img.shape
    kernel_2d = kernel_2d.view(1, 1, ksize, ksize).repeat(C, 1, 1, 1)    
    pad = (ksize - 1) // 2 # 保持卷积前后图像尺寸不变# mode=relfect 更适合计算边缘像素的权重    
    batch_img_pad = F.pad(batch_img, pad=[pad, pad, pad, pad], mode='reflect')    
    weighted_pix = F.conv2d(batch_img_pad, weight=kernel_2d, bias=None,                            
                            stride=1, padding=0, groups=C)
    return weighted_pix
...
if __name__ == "__main__":
    ...
    img1 = img.squeeze(dim=0)
    img1 = np.array(img1)
    cv2.imwrite("output2.jpg", img1)

    img2 = CVGaussianBlur(img.float(), 5)
    img2 = img.squeeze(dim=0)
    img2 = np.array(img1)
    cv2.imwrite("output3.jpg", img1)
