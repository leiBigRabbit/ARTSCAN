...
def getGaussianKernel(ksizes, sigmac, sigmas):
    ...
    for ksize, sigc, sigs in enumerate(zip(ksizes, sigmac, sigmas)):
        ...
        Dckernel1d = np.exp(-(xs ** 2) / (2 * sigc ** 2)) # 计算一维卷积核# 根据指数函数性质，利用矩阵乘法快速计算二维卷积核    
        kernelc = Dckernel1d[..., None] @ Dckernel1d[None, ...]   #@矩阵乘法
        kernelc = torch.from_numpy(kernelc)    
        kernelc.append(kernelc / kernelc.sum()) # 归一化return kernel

        Dskernel1d = np.exp(-(xs ** 2) / (2 * sigs ** 2))
        kernels = Dskernel1d[..., None] @ Dskernel1d[None, ...] 
        kernels = torch.from_numpy(kernels)    
        kernels.append(kernels / kernels.sum()) 
    return kernels
...
if __name__ == "__main__":
    ...
    img1 = GaussianBlur(img.float(), [5,17,41], Dc = [0.3, 0.75, 2], Ds = [1, 3, 7])
