...
if __name__ == "__main__":
    ...
    # img1 = np.array(imgs)
    r = cv2.imwrite("output2.tiff", imgsc[0].squeeze(dim=0))
    # r = np.load('output2.tiff')
    cv2.imwrite("output2.jpg",np.array(imgsc[0].squeeze(dim=0)))
    cv2.imwrite("output3.jpg",np.array(imgss[0].squeeze(dim=0)))
    np.save("output4.npy",np.array(imgsc_s[0].float().squeeze(dim=0).squeeze(dim=0)))
    r = np.load('output4.npy')
    cv2.imwrite("output5.jpg",np.array(imgsccuts[0].squeeze(dim=0).squeeze(dim=0)))
    cv2.imwrite("output6.jpg",np.array(imgscadds[0].squeeze(dim=0).squeeze(dim=0)))
    
    img = cv2.imread("output4.PFM")
    print(1)
