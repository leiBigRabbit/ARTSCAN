...
def GaussianBlur(batch_img, ksize, sigmac, sigmas):    
    ...
    for ks, kern_c, kern_s in zip(ksize, kernel_c, kernel_s):
        ...
        weighted_pix = (weighted_Dcg.float() - weighted_Dcs.float())/(1 + weighted_Dcg.float() + weighted_Dcs.float()).float()
    ...
    return weighted_Dcg, weighted_Dcs, weighted_pixs, weighted_c_s, weighted_c__s
...
if __name__ == "__main__":
    ...
    imgsc, imgss, imgsc_s, imgsccuts, imgscadds = GaussianBlur(img.float(), [5,17,41], sigmac = [0.3, 0.75, 2], sigmas = [1, 3, 7])
    # cv2.imwrite("output2.jpg",np.array(imgsc[0].squeeze(dim=0)))
    cv2.imwrite("output1.tiff",np.array(imgsc[0].squeeze(dim=0)))
    cv2.imwrite("output2.tiff",np.array(imgss[0].squeeze(dim=0)))
    cv2.imwrite("output3_1.tiff",np.array(imgsc_s[0].float().squeeze(dim=0).squeeze(dim=0)))
    cv2.imwrite("output3_2.tiff",np.array(imgsc_s[0].float().squeeze(dim=0).squeeze(dim=0)))
    cv2.imwrite("output3_3.tiff",np.array(imgsc_s[0].float().squeeze(dim=0).squeeze(dim=0)))
