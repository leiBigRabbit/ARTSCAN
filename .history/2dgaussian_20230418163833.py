...
if __name__ == "__main__":
    ...
    imgs = GaussianBlur(img.float(), [5,17,41], sigmac = [0.3, 0.75, 2], sigmas = [1, 3, 7])
    # imgs = imgs[0].squeeze(dim=0)
    # img1 = np.array(imgs)
    cv2.imwrite("output2.jpg",np.array(imgs[0].squeeze(dim=0)))
    cv2.imwrite("output3.jpg",np.array(imgs[1].squeeze(dim=0)))
    cv2.imwrite("output4.jpg",np.array(imgs[2].squeeze(dim=0)))
