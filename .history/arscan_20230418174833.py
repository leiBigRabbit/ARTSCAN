...
def GaussianBlur(batch_img, ksize, sigmac, sigmas):    
    ...
    for ks, kern_c, kern_s in zip(ksize, kernel_c, kernel_s):
        ...
        weighted_pix = (weighted_Dcg.float() - weighted_Dcs.float())/(1 + weighted_Dcg.float() + weighted_Dcs.float())
        weighted_pixs.append(weighted_pix)
        weighted_c_s.append(weighted_Dcg - weighted_Dcs)
        weighted_c__s.append(1 + weighted_Dcg + weighted_Dcs)
...
if __name__ == "__main__":
    ...
    cv2.imwrite("output4.jpg",np.array(imgsc_s[0].squeeze(dim=0).squeeze(dim=0)))
