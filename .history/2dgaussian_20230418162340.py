...
def getGaussianKernel(ksizes, sigmac, sigmas):
    ...
    kernel_c = []
    kernel_s = []
    for ksize, sigc, sigs in enumerate(zip(ksizes, sigmac, sigmas)):
        ...
        kernelc = kernel1dc[..., None] @ kernel1dc[None, ...]   #@矩阵乘法
...
def GaussianBlur(batch_img, ksize, sigmac, sigmas):    
    ...
    kernel_c = [kernel_c.view(1, 1, ksize, ksize).repeat(C, 1, 1, 1) for kernel_c in kernel_c]  
    kernel_s = [kernel_s.view(1, 1, ksize, ksize).repeat(C, 1, 1, 1) for kernel_s in kernel_s]

    weighted_pixs = []
    for ks, kern_c, kern_s in enumerate(zip(ksize, kernel_c, kernel_s)):
...
if __name__ == "__main__":
    ...
    img1 = GaussianBlur(img.float(), [5,17,41], sigmac = [0.3, 0.75, 2], sigmas = [1, 3, 7])
